...
export function CurvaDistribucion() {
  ...
  const onMove = e => {
    ...
    setHoverS(x.invert(((e.clientX - r.left) / r.width) * W));
  };
  ...
  return (
    ...
    >
      ...
      <ZoomSvg viewBox={`0 0 ${W} ${H}`} label="Curva de distribución salarial">
        <path
        ...
        <rect className="fs-hit" x={X0} y={Y0 - 6} width={X1 - X0} height={Y1 - Y0 + 8} onMouseMove={onMove} onMouseLeave={() => setHoverS(null)} />
      </ZoomSvg>
    </Figure>
  );
}
...
export function Percentiles() {
  ...
  return (
    ...
    >
      <ZoomSvg viewBox={`0 0 ${W} ${H}`} label="Percentiles salariales">
...
export function EvolucionDistribucion() {
  ...
  const hi = Math.max(...datos.map(d => d.p90)) * 1.04;
  ...
  return (
    <Figure
      ...
      legend="Banda clara = del percentil 10 al 90 · banda oscura = del 25 al 75 · línea continua = mediana · puntos = media"
      source="Fuente · INE · EAES tabla 28191"
    ...
    >
      <div className="fs-readout">
        ...
        <span>
          <span className="fs-readout-k">Tu salario frente a la mediana</span>
          <span className="fs-readout-v fs-signal">{sign(bruto - act.p50)}</span>
        </span>
      </div>
      ...
      <ZoomSvg viewBox={`0 0 ${W} ${H}`} label="Evolución de la distribución salarial">
        ...
        {datos.map(d => (
          <g key={d.anio}>
            <circle cx={round(x(d.anio))} cy={round(y(d.media))} r={2.6} fill="var(--counter)" />
            <circle cx={round(x(d.anio))} cy={round(y(d.p50))} r={d.anio === act.anio ? 4.4 : 2.4} fill={d.anio === act.anio ? 'var(--signal)' : 'var(--ink)'} />
            <rect
              className="fs-hit"
              x={round(x(d.anio)) - 13}
              y={Y0 - 10}
              width={26}
              height={Y1 - Y0 + 16}
              onMouseEnter={() => setHover(d.anio)}
              onMouseLeave={() => setHover(null)}
              onClick={() => setAnio(d.anio)}
            >
              <title>{`${d.anio} — mediana ${eur(d.p50)}, P10 ${eur(d.p10)}, P90 ${eur(d.p90)}`}</title>
            </rect>
          </g>
        ))}

        {/* your salary, straight across the whole series */}
        {bruto > 0 && bruto < hi && (
          <g>
            <line x1={X0} y1={round(y(bruto))} x2={X1} y2={round(y(bruto))} stroke="var(--signal)" strokeWidth={1.2} strokeDasharray="4 3" />
            <Label x={X1 + 8} y={round(y(bruto)) + 3} size={9.5} weight={700} color="var(--signal)" mono>
              TU SALARIO
            </Label>
          </g>
        )}

        {[['p90', 'P90'], ['p75', 'P75'], ['p50', 'MEDIANA'], ['p25', 'P25'], ['p10', 'P10']].map(([k, l]) => (
          <Label key={k} x={X1 + 8} y={round(y(ultimo[k])) + 3} size={9} color="var(--ink-4)" mono>
            {l}
          </Label>
        ))}
      ...
      <p className="fs-note" style={{ marginTop: 12, maxWidth: '72ch' }}>
        {real
          ? 'En euros constantes la banda apenas se ensancha: el reparto salarial español se ha movido poco en quince años, y lo que ha cambiado lo ha hecho sobre todo en la parte baja, empujada por el SMI.'
          : 'En euros corrientes todo sube, pero buena parte de esa subida es sólo inflación. Cambia a euros constantes para ver el movimiento real.'}
      </p>
    </Figure>
  );
}
